...
func RunTacticalTest(cfg models.TacticalConfig, ctx context.Context, client *http.Client) {
	// Using Electric and Bold for the engagement line
	fmt.Printf("\n%s%s[GHOST_MODE] %sENGAGING %s OMNI-VECTOR -> %s:%s%s\n", 
		output.Electric, output.Bold, output.NeonPink, cfg.Vector, cfg.Target, cfg.Port, output.Reset)
...
func executeVector(cfg models.TacticalConfig, client *http.Client) {
	...
	case "UDP":
		...
		if conn != nil { conn.Write(make([]byte, 1350)); conn.Close() }
	case "ACK":
