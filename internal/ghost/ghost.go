...
func NewGhostTransport(proxyAddr string) (*http.Transport, error) {
	...
	return ghostTrans, initErr
}

// GetTransport returns the configured Ghost transport if enabled, or nil for direct connections.
func GetTransport(enabled bool) *http.Transport {
	if !enabled {
		return nil
	}
	trans, _ := NewGhostTransport("")
	return trans
}
